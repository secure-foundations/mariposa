...
def setup_build_core(subparsers):
    p = subparsers.add_parser(
        "build-core",
        help="create core query form a given query (by performing mutations)",
    )
    ...
    p.add_argument(
        ...
        default=False,
        action="store_true",
        help="try to complete the core query after building it",
    )
    p.add_argument(
        "--ids-available",
        default=False,
        action="store_true",
        help="whether ids are available in the query",
    )
    p.add_argument(
        "--keep-quantified",
        default=False,
        action="store_true",
        help="keep quantified variables in the core query",
    )


def setup_convert_smt_lib(subparsers):
    p = subparsers.add_parser(
        "convert-smtlib", help="convert a verus query to smt-lib standard (cvc5) format"
    )
    add_input_query_option(p)
    add_output_query_option(p)
...
def setup_get_inst(subparsers):
    p = subparsers.add_parser(
        "get-inst", help="get instantiations from a query (using cvc5)"
    )
    add_input_query_option(p)
    add_output_log_option(p)
    add_timeout_option(p)
...
def setup_emit_quake(subparsers):
    ...
    add_timeout_option(p)
    parser.add_argument(
        "--quake-count", default=4, help="number of iterations to perform quake"
    )


def setup_check_subset(subparsers):
    p = subparsers.add_parser(
        "check-subset", help="check if a query is a subset of another"
    )
    add_input_query_option(p)
    p.add_argument(
        "--subset-query",
        required=True,
        help="the query that is supposed to be a subset",
    )
...
def setup_inst_z3(subparsers):
    p = subparsers.add_parser(
        "inst-z3", help="use trace and core to build a reduced query"
    )
    add_input_query_option(p)
    add_output_query_option(p)
    add_restart_option(p)
...
def setup_trace_debug(subparsers):
    p = subparsers.add_parser("debug-trace", help="debug trace")
    add_input_query_option(p)
    add_input_log_option(p)
...
if __name__ == "__main__":
    ...
    if args.sub_command == "build-core":
        if args.complete:
            ...
            log_check(
                m.run(),
                f"failed to use mutants {args.solver} on {args.input_query_path}, no core log created",
            )
    elif args.sub_command == "convert-smtlib":
        convert_smtlib(args.input_query_path, args.output_query_path, args.incremental)
        ...
        check_lfsc_proof(
            args.input_log_path, args.output_log_path, args.timeout, args.clear_existing
        )
        ...
        emit_quake_query(
            args.input_query_path, args.output_query_path, args.quake_count
        )
        ...
        log_check(
            is_assertion_subset(args.input_query_path, args.subset_query),
        ...
        handle_inst_z3(
            args.input_query_path, args.output_query_path, args.timeout, args.restarts
        )
