...
from utils.system_utils import list_smt2_files, print_banner, subprocess_run
...
class WomboAnalyzer:
    ...
    def step_1_2(self, group: ProjectGroup):
        ...
        for qid in core.qids:
            ...
            if s != STB.UNSOLVABLE:
                continue
            i_path = base_p.get_ext_path(qid)
            c_path = core_p.get_ext_path(qid)
            o_path = extd_p.get_ext_path(qid)
            if os.path.exists(o_path):
                continue
